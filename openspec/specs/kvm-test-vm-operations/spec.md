# kvm-test-vm-operations Specification

## Purpose
为真实 KVM 测试提供可重复且可审计的项目架构驱动 VM 生命周期操作, 使节点、模板、网络和 guest 布局变化不会导致流程依赖过期记忆.
## Requirements
...
#### Scenario: Historical roles are not reused implicitly

- **WHEN** historical inventory contains a VM role absent from the current project plan
- **THEN** the skill SHALL not create or reuse that role implicitly
...
### Requirement: Expand the cloned virtual disk to planned capacity
