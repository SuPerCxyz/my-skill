...
3. 保存前删除密码、token、密钥、cookie、完整连接串和不必要的用户数据; 只保留
   恢复任务所需的脱敏摘要
4. 可将恢复摘要写入 `/tmp/claude-context-handoff.md`, 写入前设置 `umask 077`,
   确保文件权限不宽于 `0600`; 不默认写入项目目录
5. 用户执行 `/clear` 后, 仅加载该恢复摘要
6. 恢复完成后删除临时摘要; 无法删除时明确告知用户路径
7. 禁止重新读取已经总结过的所有文件
8. 仅恢复当前未完成步骤
