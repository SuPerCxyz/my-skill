...
1. 检查 `agent-browser` 是否可用;不可用时先安装，安装失败则停止真实 UI 测试。
...
3. 如果本机缺少 `agent-browser`，必须先安装;安装或启动失败时返回阻塞，不继续执行 UI 测试。
...
16. 测试中所有可原子化的操作必须从 `patterns/` 调用，不在用例中重复编写页面交互逻辑。
17. 列表页操作按钮可能在首屏渲染后由权限、配额或数据加载异步放开;创建、
    挂载、删除等动作前必须等待目标按钮 enabled，超时后才判定为环境阻塞。
18. 按钮持续 disabled 时，先检查 `.projects-switch-wrapper` 项目是否与用例
    或 `resources.project_name` 一致;项目错误不能直接判定为权限或配额问题。
19. 需要探索新模块时，优先记录当前 URL、页面标题、列表字段、顶部按钮、行内动作、
    更多操作、勾选前后按钮状态，再决定是否执行真实提交。
20. 发生操作失败时，不要只记一句“click failed”;至少补齐:
    页面、路径、使用的定位方式、错误现象、截图路径、console、errors、
    network requests、原因分析、修复后的正确做法。
21. 如果菜单、dropdown、service catalog、modal 覆盖了目标元素，优先处理覆盖层，
    不要直接 force click。
22. interactive snapshot 可能漏掉禁用菜单项或纯文本提示;探索更多操作时，必要时
    同时读取 `agent-browser get text body`。
23. 创建云主机必须把基础配置、网络配置、系统配置、最终确认合并为一次
    `agent-browser eval --stdin` 或一次 `agent-browser batch`，只在提交后轮询
    结果;不要每个下一步都回传给用户或依赖多次 snapshot。
24. 创建云主机登录凭证优先级:同时设置密码和密钥 > 密码 > 密钥 > 自动生成
    密码并使用密码登录。自动生成的密码只写入运行报告，不写入环境文件。
25. 创建云主机系统盘大小必须在基础配置页的 `*Root Disk` 区域设置，并在进入
...
27. 除非用户明确要求，Codex 场景不升级 `gpt-5.3-codex` 或 `xhigh`。
28. 长耗时用例被取消、中断或切回主任务后，不得假设最后一个异步动作成功;
    必须重新通过页面列表确认资源状态后再继续、清理或下结论。
29. 用户取消测试时只整理已验证事实、已创建资源、最后确认点和可沉淀规则;
    未经用户明确要求，不继续执行后续步骤，也不主动清理测试资源。
30. 清理资源前必须先向用户说明待清理资源清单、清理顺序和影响范围;未得到
...
- Codex 场景:默认模型 `gpt-5.4-mini`，`reasoning_effort` 使用 `medium`。
- Claude Code / OpenCode 场景:按当前会话模型和推理强度执行，无需指定。
...
执行过程中发现以下情况时，必须更新 skill 文档，而不是只把操作留在会话里:
