...
23. 除非用户明确要求，Codex 场景不升级 `gpt-5.3-codex` 或 `xhigh`。
23. 长耗时用例被取消、中断或切回主任务后，不得假设最后一个异步动作成功；
    必须重新通过页面列表确认资源状态后再继续、清理或下结论。
24. 用户取消测试时只整理已验证事实、已创建资源、最后确认点和可沉淀规则；
    未经用户明确要求，不继续执行后续步骤，也不主动清理测试资源。
25. 清理资源前必须先向用户说明待清理资源清单、清理顺序和影响范围；未得到
